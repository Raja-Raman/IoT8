...
bool FlashHelper::writeFlash () {    
    ...
    block.extra1 = (long)pC->relay_enabled;
    block.extra2 = (long)pC->buzzer_enabled;  
    block.extra3 = 0L;  // unused for now  
...
/* 
   ...
   The device id, group id and extra param are stored in the config object pC.
*/

bool FlashHelper::readFlash () {
    DeviceIdentifier ident1, ident2, ident3;  
    if (!readBlock(BLOCK_ADDRESS1, ident1)) {  // true if the block is OK
        SERIAL_PRINTLN ("Block 1 is corrupted");      
        return false;
    }
    if (!readBlock(BLOCK_ADDRESS2, ident2)) {  
        SERIAL_PRINTLN ("Block 2 is corrupted");      
        return false;  
    }
    if (!readBlock(BLOCK_ADDRESS3, ident3)) {  
    ...
    // TODO: compare the 3 extra values also
    pC->device_id = ident1.device_id;
    pC->group_id = ident1.group_id;    
    pC->relay_enabled = (bool)temp_extra1;
    pC->buzzer_enabled = (bool)temp_extra2;
    return true;
}
...
bool FlashHelper::readBlock (int block_addr, DeviceIdentifier& ident) {
    ...
    EEPROM.get (block_addr, block);
        ...
        SERIAL_PRINT("extras: ");
        SERIAL_PRINT(block.extra1);     
        SERIAL_PRINT(", ");
        SERIAL_PRINT(block.extra2);    
        SERIAL_PRINT(", ");
        SERIAL_PRINTLN(block.extra3);                    
        SERIAL_PRINTLN("-------------------------");   
    #endif    
    ...
    // stored in a local variable, pending validation:
    temp_extra1 = block.extra1;
    temp_extra2 = block.extra2;
    temp_extra3 = block.extra3;    
    return(true);
}
